...
export function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <div className="group relative flex flex-col overflow-hidden rounded-xl border border-border bg-card shadow-[var(--shadow-card)] transition-all hover:shadow-[var(--shadow-elegant)] hover:-translate-y-1">
      <Link to="/product/$id" params={{ id: product.id }} className="relative aspect-[4/3] overflow-hidden bg-muted">
        ...
        {sold && (
          <Badge className="absolute top-2 right-2 bg-destructive text-destructive-foreground">Sold Out</Badge>
        )}
